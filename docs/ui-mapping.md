...
| PageStatus | ⚠️ | ✅ | **兩端都有整頁狀態元件，版面與文案也對得上**（插圖 120、標題 `heading-small`、說明 `body-large`、都是 `content-subtle`，間距 16/4/16）。Android 的 `EmptyCompose` 有 `EmptyType` 五個型別，**與設計系統的五種 status 一對一**；iOS 是 `EmptyView` 加自由填入的 `EmptyViewModel`，**沒有 status 列舉**，每個呼叫端自己挑圖配文案。**真正的落差在插圖透明度的做法**：設計系統與 Android 都在**元件層**統一套 40%（CSS `opacity` / Compose `alpha`），圖檔本身不帶透明度；**iOS 元件完全不設 alpha，改由每張圖檔自己烘**——7 張裡有 2 張沒烘（`imgStatusEmpty`、`imgContentNotExist`），所以**票券詳情、集點任務詳情、對獎頁三個畫面的插圖是 100% 實色**，比其他狀態頁深一階。文案兩端逐字相同，與設計系統預設只差「無法顯示頁面」vs「無法顯示此頁面」——**已裁決以「無法顯示頁面」為準，兩端改** | 直接標「整頁狀態」＋狀態種類即可，**不需要標插圖透明度**——那是元件的事。**iOS 的透明度落差已列入待修**（見複核報告 C-27）。**「暫時沒有內容」已裁決不定預設文案**，設計稿每次自己給標題與說明；其餘四種可直接沿用預設。**iOS 也已裁決不補 status 列舉**，維持由呼叫端配圖與文案 |

## 頁面框架類

| 元件 | iOS | Android | 現況 | 處理方式 |
|------|:---:|:---:|------|---------|
| NavigationBar | ⚠️ | ✅ | **Android 收得很乾淨**：`core/designsystem` 有一支底層 `AppBar`，對外三個入口——`TopBackAppBar`（返回）／`TopCloseAppBar`（關閉）／`TopTitleAppBar`（無返回），每支都有 **Regular／Large 兩種樣式** 與 **Default／Home 兩種型**（Large-Home 就是標題靠左那種），34 個畫面共用；只有登入與 OTP 兩頁還吃 2019 的 XML `invos_simple_toolbar`（色票對、字級硬寫 16 bold）。<br>**iOS 沒有共用元件**（`INVOSUI` 裡沒有任何導覽列），**五條路徑並存**：①**系統列 token 版** `ViewControllerNavigationDecoratable`（2023，34 檔，`labelLarge` + `content-bold` + `background-default`，SwiftUI 畫面經 `SwiftUINavigationBar` 也走這條）②**系統列 2019 舊版** `SwiftBaseViewController.setupNavgationBar`（6 畫面：城市地圖、帳號授權 ×2、推薦好友、選擇會員卡、Logging，硬寫 17pt semibold、預設白字、舊色 `backgroundInvosMedium` / `primaryBlack`）③**自刻 UIView 列** `PlainNavBarView`（2026/5，4 畫面：消費分析 ×2、會員卡詳情、會員卡排序，高 56、17pt semibold、字色固定 `foreverWhite`，給彩底頁用）④**三個 tab 各自的自刻 UIView 列**，高 56、色票正確——`HomeCustomNavBarView`（首頁，**沒有標題**：只有 logo + 常見問題 / 設定 / 行為紀錄，捲動才浮出底色）、`CarrierCustomNavBarView`（載具管理：`display` 大標題 + 常見問題 / 重新整理 / 排序，右側按鈕依條碼・載具・會員卡三個分頁換）、`InvoiceBookCustomNavBarView`（兩種模式：`invoiceBook` 用在「我的發票」，`display` 大標題 + 搜尋 / 匯出 / 更多 / 篩選；`carrier(title:)` 用在從載具管理點進單一載具的發票清單，改成返回鍵 + `labelLarge` 置中標題，**不是大標題**）⑤**系統列整條隱藏、頂部列做成內容區的第一個 stack**：集點兌禮（`RewardVC`）與掃描對獎（`InvoiceScanVC`）兩個 tab，`display` 標題靠左 + 右側按鈕，內距上下 10 左右 16。<br>兩處全平台落差：**iOS 沒有系統大標題**（`prefersLargeTitles` 全部為 false），**8 個畫面的大標題全是手刻，而且分三種做法**：放在自刻導覽列裡（我的發票、載具管理）、系統列隱藏後放內容區 stack（集點兌禮、掃描對獎）、系統列留著但標題留空、大標題放內容區（我的物品、桌面小工具、自訂款載具、載具登入的登入模式——後三者標題下方還自己畫了 1pt 線，性質同 Android Large 的分隔線）。**只有「我的物品」做了捲動收合**（大標題收起、系統列補上 `labelLarge` 標題，0.4 秒動畫），其餘捲動時大標題就跟著內容捲掉；內距也不統一（上 4 與上下 10 都有）。**iOS 的底線不是導覽列畫的**——不是畫不出來（系統列的 `appearance.shadowColor` 就是那條線，`configureWithOpaqueBackground()` 預設也帶著，開發用的 BehaviorLog 頁沒關掉就仍有系統線），是這個 app 三處 appearance 都主動設成 `nil` / `.clear`、自刻列也沒有加。實機看到的那條是**內容區自己在最上緣補的 1pt `border-subtle`**，共 18 個畫面這樣做（`UIView.setTopDivider()` 15 處 + 匯款帳戶驗證 / 編輯 / 發票分類 3 處自寫），沒補的畫面就真的沒有線；主 tab 裡「載具管理」那條是 `SegmentedView`（Tabs）自己的底線，「我的發票」與「首頁」沒有線。Android 則是 `showDivider` 參數，預設 true。另有兩項數值差異屬平台慣例／規格未落地：**一般列高 iOS 44、Android 64（M3）**；**大標題字重規格是 Bold 700，iOS 與 Android 都做成 Medium 500** | **設計稿標「一般標題／大標題」＋「底線有／無」即可，不要標高度與字重**——高度按平台慣例走，字重待裁決（規格 700、兩端都 500，建議改規格為 500）。**iOS 要指明走哪一條**：彩底頁（分析、會員卡）是 ③、五個 tab 各自不同（我的發票 / 載具管理 / 首頁走 ④，集點兌禮 / 掃描對獎走 ⑤）、其餘走 ①；**②那 6 個舊畫面改動時外觀會從 17pt 白字變 16pt `content-bold`，需重新出圖**。<br>**兩項需另外處理**：**iOS 沒有大標題導覽列**——8 個畫面各刻一份、內距與收合行為都不同，設計稿標大標題時要一併指明**捲動時要不要收合**（目前只有我的物品有），或提出收斂需求；**iOS 的底線目前要逐頁交代**——現況它屬於內容區而不是導覽列，沒特別標就不會有（與 [component-usage.md](./component-usage.md) 分隔線形態① 相關）；收斂時可以直接讓導覽列自己畫（`shadowColor` 綁 `border-subtle`），不必每頁補。**iOS 五條路徑收斂成一支共用元件建議列入待修**（尚未編號）。search／tabs 兩型兩端都是在畫面層自己把搜尋欄或 Tabs 接在導覽列下方，不是元件參數，設計稿照常標即可 |
| TabBar | ⚠️ | ⚠️ | **兩端都是平台原生容器，而且整個 app 各只有一座**：iOS 是 `UITabBarController` 子類（`InvoiceMainViewController`）、Android 是 Material `BottomNavigationView`（`activity_main.xml`）。樣式都已收斂在單一處（iOS `setupTabBarAppearance()`，換主題時重跑；Android `MainActivity.initView()`），**五個分頁的順序與圖示兩端一致**，**選中圖示都有線框／實心兩套**（iOS `selectedImage`、Android selector drawable，都是 24），**顏色三邊逐項吻合**（未選 `content-subtlest`、選中圖示 `content-brand-default`、選中文字 `content-bold`、底色 `background-default`）。落差三處：**標籤字級 iOS 走系統預設（10、選中不變粗）、Android 明寫 12sp 且選中轉粗**；**紅點兩端都沒用 token**（iOS 把 `●` 字元塞進系統 badge、上系統紅，Android 用舊色 `invosError`），**帶數字的版本兩端都沒有在用**；**上緣分隔線 Android 明畫 1dp `border-subtle`、iOS 用系統預設 hairline** | **不必做成共用元件**——兩端都只有一座、樣式已集中在一處，再封裝沒有實益。設計稿標「底部導覽」即可，不需附版面規格。**字級已裁決以 10 為準**（設計系統與 iOS 現況即符合，**Android 的 12sp 要改成 10sp**）；紅點色與分隔線色的落差併入待修（紅點見複核報告 C-19） |
